...
describe("SignMessage factory (BCUR2 case)", () => {
  // This block lives in signmessage.test.ts rather than
  // interactions.test.ts so the default BCUR2Encoder construction
  // path runs unmocked — interactions.test.ts mocks `../encoder` at
  // module scope, which would replace the encoder used here with a
  // stub that has no `qrFragments` getter.
